...
def start_test():
    ...
    progress_bar = ttk.Progressbar(game_screen, orient="horizontal", length=800, mode="determinate", maximum=initial_time)
    ...
    update_button = tk.Button(button_frame, text="Update", command=update_grid, font=("Arial", 16), width=10, height=2, bg="Yellow")
    ...
    def test():
        ...
        percentage_left = (remaining_time / initial_time) * 100

        if percentage_left <= 10:
            progress_bar['style'] = "red.Horizontal.TProgressbar"
        elif percentage_left <= 25:
